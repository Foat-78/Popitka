...
{
    if (a.GetLength(1) != b.GetLength(0)) throw new Exception("Нельзя умножить такие матрицы"); //где, throw new Exception - бросить новое иссключение
...
void PrepareParallelMatrixMul(int[,] a, int[,] b)//метод для запуска параллельно потоки
{
    if (a.GetLength(1) != b.GetLength(0)) throw new Exception("Нельзя умножить такие матрицы");
    int eachThreadCalc = N / THREADS_NUMBER;//переменная сколько вычеслении будет приходит на поток 
    Thread[] arr = new Thread[2];
    var threadsList = new List<Thread>();// где, <Thread>() -  массив улучшеный
    for (int i = 0; i < THREADS_NUMBER; i++)
    {
        ...
        threadsList.Add(new Thread(() => ParallelMatrixMul(a, b, startPos, endPos)));//где, => - анонимная функция (вставление функции). В данной строке сщзданные потоки передаются в функцию для вычисления
        threadsList[i].Start();// где, Start() - запустить поток
    }
    for (int i = 0; i < THREADS_NUMBER; i++)
    {
        threadsList[i].Join();// где, Join() - подождать
    }
}
...
bool EqualityMatrix(int[,] fmatrix, int[,] smatrix)// метод для сравнения двух матриц
{
    bool res = true;
